...
trait Operator[A] {
  ...
  def execute(operands: Option[A]*): Either[Option[String], Option[List[A]]]
}

class Operand[A] extends Operator[A] {
  override def getNumberOperator: Int = 0

  override def execute(operands: Option[A]*): Either[Option[String], Option[List[A]]] = Right(None)
}
...
abstract class IfOperator[A](val left: Option[String], val right: Option[String]) extends UnaryOperator[A] {
  ...
  override def execute(operands: Option[A]*): Either[Option[String], Option[List[A]]] = {
    operands.head match {
